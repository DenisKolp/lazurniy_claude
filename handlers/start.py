    ...
    async with async_session_maker() as session:
        ...
        if user:
            if user.status == UserStatus.VERIFIED:
                ...
                reason = user.rejected_reason or "Не указана"
                keyboard = [
                ...
                await update.message.reply_text(
                    ...
                    f"Причина: {reason}\n\n"
                    "Нажмите кнопку ниже для повторной подачи заявки.",
                    reply_markup=reply_markup
                )
        else:
            keyboard = [
...
5. Контакты:
Email: support@lazurny-kp.ru
Телефон: см. в разделе Помощь

Полная версия доступна в файле PRIVACY_POLICY.md
    ...
    if update.message.contact:
        ...
        if validated_phone:
            ...
            await update.message.reply_text(
                ...
                "Пожалуйста, загрузите документы, подтверждающие ваше право собственности:\n"
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        ...
        await update.message.reply_text(
            ...
            "Нажимая кнопку '🔐 Пройти верификацию', вы соглашаетесь с обработкой ваших персональных данных "
            "в соответствии с нашей Политикой конфиденциальности.\n\n"
            "Используйте /privacy для просмотра полной политики конфиденциальности.",
            reply_markup=reply_markup,
            parse_mode='Markdown'
        )
